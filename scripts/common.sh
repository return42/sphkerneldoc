...
# ----------------------------------------------------------------------------
    ...
    # usage: insert_index_files <src-doc-folder>
    ...
    for src_index in $(find ./ -type d ); do
        ...
        if [[ -z "$src_index" ]]; then
            ...
            TITLE="$(basename "$src_index")"
        fi

        [[ 0 < $VERBOSE ]] && info_msg "create '${src_index}/index.rst'"

        {
            render_template "$TEMPLATES/index.rst"
            echo ""

        } > "$src_index/index.rst"

        pushd "$src_index" > /dev/null

        for rst_file in $(find ./ -maxdepth 1 -name '*.rst'); do

            rst_file="${rst_file:2}"
            [[ "index.rst" == $rst_file ]] && continue
            [[ -z $rst_file ]] && continue
            fname="${rst_file%.*}"
            echo "   $fname"
        ...
        for rst_file in $(find ./ -maxdepth 1 -type d); do
