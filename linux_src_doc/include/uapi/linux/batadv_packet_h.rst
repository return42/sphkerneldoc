...
.. src-file: include/uapi/linux/batadv_packet.h

.. _`batadv_tp_is_error`:

batadv_tp_is_error
==================

.. c:function::  batadv_tp_is_error( n)
...
0 when not error was detected, != 0 otherwise
...
.. _`batadv_icmp_packettype`:

enum batadv_icmp_packettype
===========================

.. c:type:: enum batadv_icmp_packettype

    ICMP message types

.. _`batadv_icmp_packettype.definition`:
...
.. _`batadv_icmp_packettype.constants`:

Constants
---------

BATADV_ECHO_REPLY
    success reply to BATADV_ECHO_REQUEST

BATADV_DESTINATION_UNREACHABLE
    failure when route to destination not found

BATADV_ECHO_REQUEST
    request BATADV_ECHO_REPLY from destination

BATADV_TTL_EXCEEDED
    error after BATADV_ECHO_REQUEST traversed too many hops

BATADV_PARAMETER_PROBLEM
    return code for malformed messages

BATADV_TP
    throughput meter packet
...
.. _`batadv_bla_claimframe`:

enum batadv_bla_claimframe
==========================

.. c:type:: enum batadv_bla_claimframe

    claim frame types for the bridge loop avoidance

.. _`batadv_bla_claimframe.definition`:
...
.. _`batadv_bla_claimframe.constants`:

Constants
---------

BATADV_CLAIM_TYPE_CLAIM
    claim of a client mac address

BATADV_CLAIM_TYPE_UNCLAIM
    unclaim of a client mac address

BATADV_CLAIM_TYPE_ANNOUNCE
    announcement of backbone with current crc

BATADV_CLAIM_TYPE_REQUEST
    request of full claim table

BATADV_CLAIM_TYPE_LOOPDETECT
    mesh-traversing loop detect packet
