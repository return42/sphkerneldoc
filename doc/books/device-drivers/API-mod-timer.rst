...
Note that if there are multiple unserialized concurrent users of the same timer, then ``mod_timer`` is the only safe way to modify the timeout, since ``add_timer`` cannot modify an
