...
The driver's ``attach`` function may block. The port that ``attach`` is given will be valid for the duration of the callback, but if the driver wants to take a copy of the pointer
it must call ``parport_get_port`` to do so. Calling ``parport_register_device`` on that port will do this for you.

The driver's ``detach`` function may block. The port that ``detach`` is given will be valid for the duration of the callback, but if the driver wants to take a copy of the pointer
it must call ``parport_get_port`` to do so.
