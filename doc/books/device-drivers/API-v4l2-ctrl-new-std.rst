...
If the ``v4l2_ctrl`` struct could not be allocated, or the control ID is not known, then NULL is returned and ``hdl``->error is set to the appropriate error code (if it wasn't set
already).
