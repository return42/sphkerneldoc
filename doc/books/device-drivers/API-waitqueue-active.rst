...
wake_up(wq); break; ``schedule``; } finish_wait( ``wq``, ``wait``);

Because without the explicit ``smp_mb`` it's possible for the ``waitqueue_active`` load to get hoisted over the ``cond`` store such that we'll observe an empty wait list while the
waiter might not observe ``cond``.
