...
this function creates and allocates a guaranteed size, preallocated memory pool. The pool can be used from the ``mempool_alloc`` and ``mempool_free`` functions. This function might
sleep. Both the ``alloc_fn`` and the ``free_fn`` functions might sleep - as long as the ``mempool_alloc`` function is not called from IRQ contexts.
