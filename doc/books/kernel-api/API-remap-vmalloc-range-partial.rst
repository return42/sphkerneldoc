...
This function checks that ``kaddr`` is a valid vmalloc'ed area, and that it is big enough to cover the range starting at ``uaddr`` in ``vma``. Will return failure if that criteria
isn't met.
