...
using ``list_empty_careful`` without synchronization can only be safe if the only activity that can happen to the list entry is ``list_del_init``. Eg. it cannot be used if another
