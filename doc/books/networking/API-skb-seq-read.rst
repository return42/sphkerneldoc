...
specified to ``skb_prepare_seq_read``. Assigns the head of the data
block to ``data`` and returns the length of the block or 0 if the end of
the skb data or the upper offset has been reached.

The caller is not required to consume all of the data returned, i.e.
``consumed`` is typically set to the number of bytes already consumed
and the next call to ``skb_seq_read`` will return the remaining part of
the block.
...
The size of each block of data returned can be arbitrary, this
limitation is the cost for zerocopy sequential reads of potentially non
linear data.
