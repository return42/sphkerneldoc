...
In the case of a 802.3-c22 PHY, reads the ID registers of the PHY at
``addr`` on the ``bus``, stores it in ``phy_id`` and returns zero on
success.

In the case of a 802.3-c45 PHY, ``get_phy_c45_ids`` is invoked, and its
return value is in turn returned.
