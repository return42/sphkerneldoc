...
If the INQUIRY is successful, zero is returned and the INQUIRY data is in ``inq_result``; the scsi_level and INQUIRY length are copied to the scsi_device any flags value is
stored in ⋆\ ``bflags``.
