...
Create and add one dev_info entry for ``vendor``, ``model``, ``strflags`` or ``flag``. If ``compatible``, add to the tail of the list, do not space pad, and set
devinfo->compatible. The scsi_static_device_list entries are added with ``compatible`` 1 and ``clfags`` NULL.
