...
Scans all the CRTCs and planes in ``dev``'s mode_config. If they're using ``fb``, removes it, setting it to NULL. Then drops the reference to the passed-in framebuffer. Might take
the modeset locks.
