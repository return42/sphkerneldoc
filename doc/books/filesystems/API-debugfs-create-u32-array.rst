...
This function creates a file in debugfs with the given name that exports ``array`` as data. If the ``mode`` variable is so set it can be read from. Writing is not supported. Seek
within the file is also not supported. Once array is created its size can not be changed.
